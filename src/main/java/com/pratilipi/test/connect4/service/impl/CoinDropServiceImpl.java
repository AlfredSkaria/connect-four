...
public class CoinDropServiceImpl implements CoinDropService{
	...
	public String dropACoin(String col, String playerId) {
		...
		if(optPlMat.isPresent()) {
	        ...
	        if(checkHorizontal(coin, matrix) 
		        || checkVertical(coin, matrix)
		        || checkForwardDiagonal(coin, matrix)
		        || checkBackwardDiagonal(coin, matrix)) {
	        	return color.equals(Connect4Constants.PLAYER_YELLOW) ? "Yellow " + Connect4SuccessMsg.GAME_WON_MESSAGE : "Red " + Connect4SuccessMsg.GAME_WON_MESSAGE;
	        }
		}else {
			return Connect4ErrorMsg.START_NEW_GAME;
		}
		
		return color.equals(Connect4Constants.PLAYER_YELLOW) ? "Red player's turn" : "Yellow player's turn";
	}
	...
	private boolean checkHorizontal(CoinModel coin, String mat[][]) {
		...
		for(int row = coin.getFocalRow(), col = coin.getMinCol(); col <= coin.getMaxCol(); col++) {
			...
			&& isValidRowCol(row, col + 1) 
			&& isValidRowCol(row, col + 2) 
			&& isValidRowCol(row, col + 3) ) {
				if(color.equals(mat[row][col]) 
					&& color.equals(mat[row][col + 1])
					&& color.equals(mat[row][col + 2])
	...
	private boolean checkVertical(CoinModel coin, String mat[][]) {
		...
		for(int col = coin.getFocalCol(), row = coin.getMinRow(); row <= coin.getFocalRow(); row++) {
			if(isValidRowCol(row, col)
			&& isValidRowCol(row + 1, col)
			&& isValidRowCol(row + 2, col)
			&& isValidRowCol(row +3, col)) {
				if(color.equals(mat[row][col]) 
					&& color.equals(mat[row + 1][col])
					&& color.equals(mat[row + 2][col])
	...
	private boolean checkForwardDiagonal(CoinModel coin, String mat[][]) {
		...
		for(; row <= coin.getMaxRow() && col <= coin.getMaxCol(); row++, col++) {
			if(isValidRowCol(row, col) && isValidRowCol(row + 1, col + 1)
			&& isValidRowCol(row + 2, col +2) && isValidRowCol(row + 3, col + 3)) {
				if(color.equals(mat[row][col])
				&& color.equals(mat[row + 1][col + 1])
				&& color.equals(mat[row + 2][col + 2])
				&& color.equals(mat[row + 3][col + 3])){
					 return true;
				}
			}
		}
	...
	private boolean checkBackwardDiagonal(CoinModel coin, String mat[][]) {
		...
		for(; row >= coin.getMinRow() && col <= coin.getMaxCol(); row--, col++) {
			if(isValidRowCol(row, col)
			&& isValidRowCol(row - 1, col + 1)
			&& isValidRowCol(row - 2, col + 2)
